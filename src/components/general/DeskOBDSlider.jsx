    ...
    return (
        ...
        >
            ...
            >
                {/* Slide 1 */}
                <div>
                    <div style={{ maxWidth: 352 }} className={styleODB.slide_item_wrapper}>
                        ...
                        {/* <Button type="primary" style={{ minWidth: 117, width: 117, marginTop: 32 }} size="large">
                            {data.partner.more}
                        </Button> */}
                    </div>
                </div>
                        ...
                        <Row gutter={16}>
                            {data.safety_security.items.map((item, index) => (
                                <Col key={index} span={6}>
                                    ...
                                    >
                                        <div className={styleODB.safety_security_item_icon}>{item.icon}</div>
                                        <h4 className={styleODB.safety_security_item_description}>
                                            {item.description}
                                        </h4>
                                    </div>
                                </Col>
                ...
                <div>
                    <div className={styleODB.slide_item_wrapper} style={{ maxWidth: '770px' }}>
                        ...
                        <div className={styleODB.system_heading}>{data.system.top_heading}</div>
                        <div className={styleODB.system_items_wrapper}>
                            {data.system.items.map((item, index) => (
                                <div key={index} className={styleODB.system_items_item}>
                                    {item.icon}
                                    <h4 style={{ color: '#FFFFFF', fontSize: '20px', fontWeight: 500 }}>
                                        {item.heading}
                                    </h4>
                                </div>
                            ))}
            ...
            {current !== 4 && (
                <Button
                    className={`${styleODB.arrow_button} ${styleODB.arrow_button__next}`}
                    shape="circle"
                    icon={<RightOutlined />}
                    onClick={handleNextSllide}
                />
            )}
            {current !== 0 && (
                <Button
                    className={`${styleODB.arrow_button} ${styleODB.arrow_button__prev}`}
                    shape="circle"
                    icon={<LeftOutlined />}
                    onClick={handlePrevSllide}
                />
            )}
        </div>
    );
