...
import EmailQuestion from '@/components/home/EmailQuestion';
                    ...
                    <Content width="59.6%" className={styleLogin.content}>
                        <div className={styleLogin.content_wrapper}>
                            <div className={styleLogin.main_logo}>
                                <Image src="/images/logo_creditvn.svg" width={239} height={70} alt="Logo creditvn" />
                            </div>
...
function UserForm() {
    ...
    const next = (data) => {
        if (data && data.type && data.type == 'id_card' && data.value.length == 9) setCurrent(current + 2);
            ...
            request
                .post('/update_scoring', { ...new_user, address: data.value })
    ...
    const prev = (type) => {
        if (type && type == 'address' && new_user.id_card.length == 9) setCurrent(current - 2);
    ...
    return screen.md ? (
        ...
        <Layout
            style={{
                ...
                right: 0,
                background:
                    'radial-gradient(123.18% 99.53% at 130.47% 0%, rgba(253, 67, 99, 0.8) 0%, rgba(45, 34, 155, 0.8) 100%)',
